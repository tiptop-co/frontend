...
Авторизация выполняется по **номеру телефона** и паролю. Телефон является уникальным идентификатором пользователя. Пользователь не может самостоятельно изменить номер телефона — для этого необходимо обратиться к менеджеру (для официанта) или к администратору TipTop (для менеджера).
...
|6| Выбрать конкретные неоплаченные позиции и оплатить их | Оставить чаевые|
|7| Вызвать официанта | - |

- Чаевые адресуются привязанному к столу в данный момент официанту
- **Пожелания к заказу** (например, "Без лука") видны и посетителю (в экране заказа), и официанту (в детали стола)
- **Вызов официанта**: доступность кнопки определяется параметром `canCall`, возвращаемым бэкендом. Бэкенд может запрещать повторный вызов, пока предыдущая заявка не закрыта
- При **оплате** посетитель выбирает конкретные позиции из списка неоплаченных; кнопка оплаты отображает сумму выбранных позиций
...
- **Удаление** блюд, официантов и столов требует подтверждения через диалог
- **Категории блюд** (фильтры в меню) возвращаются бэкендом, не захардкожены на фронтенде
